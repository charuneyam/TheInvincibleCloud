...
module "ec2_nginx" {
  ...
  nginx_conf = {
    port = 80
    servers = compact([
      try(data.kubernetes_ingress_v1.eks_ingress_2048.status[0].load_balancer[0].ingress[0].hostname, ""),
      try(data.kubernetes_ingress_v1.gke_ingress_2048.status[0].load_balancer[0].ingress[0].ip, "")
    ])
  }
}
